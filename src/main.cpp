...
#if defined(NDEBUG)
# error "Horsepowercoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbf96a6ee6eb91f449b2c2689fcc33a42439e1408e37fcf1c0ebef15908f97bef");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Horsepowercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Horsepowercoin Signed Message:\n";
...
{
    // Horsepowercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Horsepowercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Horsepowercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbf96a6ee6eb91f449b2c2689fcc33a42439e1408e37fcf1c0ebef15908f97bef");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "14/09/2015 HorsePowerCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1442274114;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2191907;
        ...
        assert(block.hashMerkleRoot == uint256("0x50eb59447882a42561e2c43a435bdc521ef02586cbea9cb7d72d47ccf3d94c7f"));
...
unsigned char pchMessageStart[4] = { 0xb3, 0xd3, 0xd2, 0xcd }; // Horsepowercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HorsepowercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HorsepowercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HorsepowercoinMiner(CWallet *pwallet)
{
    printf("HorsepowercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("horsepowercoin-miner");
    ...
    try { loop {
        ...
        printf("Running HorsepowercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HorsepowercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HorsepowercoinMiner, pwallet));
}
